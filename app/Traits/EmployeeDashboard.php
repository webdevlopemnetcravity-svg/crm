...
trait EmployeeDashboard
{
    ...
    $image = $request->input('photoData'); // Get base64 image from request
    $image = str_replace('data:image/png;base64,', '', $image);
    $image = str_replace(' ', '+', $image);
    $imageData = base64_decode($image);

    $imageName = 'clockin_' . $this->user->id . '_' . now()->format('Ymd_His') . '.png';
    $path = public_path('images/clockin_photos/' . $imageName);

    file_put_contents($path, $imageData); // Save the image to the folder
    
}

            $attendance->clock_in_image = $imageName;
            $attendance->save();
