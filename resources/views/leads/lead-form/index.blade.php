            ...
            <div class="row">
                                        ...
                                        @foreach ($fields as $key => $label)
                                            <div class="row py-3 pt-4 border-bottom">
                                                <div class="col-md-3">
                                                    ...
                                                    <input type="hidden" name="sort_order[]" value="{{ $key }}">
                                                </div>
                                                <div class="col-md-5">{{ $label }}</div>
                                                <div class="col-md-4">
                                                    <div class="custom-control custom-switch">
                                                        <input type="checkbox" class="custom-control-input change-setting"
                                                               data-setting-id="{{ $key }}" checked
                                                               id="{{ $key }}">
                                                        <label class="custom-control-label f-14 cursor-pointer" for="{{ $key }}"></label>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
                ...
                <div class="col-sm-12 mt-4 mb-4">
                    <x-cards.data>
                        ...
                        <code>&lt;iframe src="{{ route('front.lead_form',[company()->hash]) }}" frameborder="0" scrolling="yes" style="display:block; width:100%; height:60vh;">&lt;/iframe&gt;</code>
                    </x-cards.data>
                    <x-cards.data>
                        <p class="f-w-500">Share Direct link</p>
                        <p class="f-12"><a href="{{ route('front.lead_form', [company()->hash]).'?styled=1' }}" target="_blank">{{ route('front.lead_form', [company()->hash]).'?styled=1' }}</a></p>
                        <p class="f-12"><a href="{{ route('front.lead_form', [company()->hash]).'?styled=1&with_logo=1' }}" target="_blank">{{ route('front.lead_form', [company()->hash]).'?styled=1&with_logo=1' }}</a></p>
                    </x-cards.data>
                </div>
            </div>
        </div>
...
<script>
    ...
    $('.change-setting').change(function () {
        ...
        var url = '{{ route('lead-form.update', ':id') }}'.replace(':id', id);
        $.easyAjax({
            url: url,
            type: "POST",
            blockUI: true,
